...
#         state = state.double().detach().numpy()
#         nq, nv = model.nq, model.nv
#         pin.computeForwardKinematicsDerivatives(model, data, state[0:nq], state[nq:nq + nv], np.zeros(nv))
#         pin.updateFramePlacements(model, data)

#         pin.getFrameVelocity(model, data, f_id, pin.ReferenceFrame.LOCAL)
